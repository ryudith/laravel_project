...
<div class="flex justify-center">
    <div class="w-8/12 bg-white p-6 rounded-lg">
        <h3 class="text-4xl mb-6 text-center">Add Pay</h3>
        ...
        <form action="{{ route('pay.add', $lend->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-4">
            ...
            <select name="status" id="status" class="bg-gray-100 border-2 w-full p-4 rounded-lg mb-6 @error('status') border-red-500 @enderror">
                <option value="0"@if(old('status') == '0') selected @endif>Cancel</option>
                <option value="1"@if(old('status') == '1' || ! old('status')) selected @endif>Accept</option>
            </select>
        ...
        <div class="mb-4">
            ...
            <textarea name="note" id="note" class="bg-gray-100 border-2 w-full p-4 rounded-lg mb-6 @error('note') border-red-500 @enderror" placeholder="Pay note">{{ old('note') }}</textarea>
